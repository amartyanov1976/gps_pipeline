...
protected:
    void SetUp() override {
        filter = std::make_unique<SpeedFilter>(300.0);
...
TEST_F(SpeedFilterTest, WorksThroughInterface) {
    IGpsFilter* ifilter = filter.get();
    auto point = createPoint(120.5);
    EXPECT_EQ(ifilter->process(point, *history), FilterResult::PASS);
    EXPECT_EQ(ifilter->getName(), "SpeedFilter");
    EXPECT_TRUE(ifilter->isEnabled());
}
