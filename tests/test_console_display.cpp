...
protected:
    ...
    GpsPoint createValidPoint() {
        ...
        p.satellites = 8;
        p.hdop = 0.9f;
        p.timestamp = 12 * 3600 * 1000 + 35 * 60 * 1000 + 19 * 1000;
        p.isValid = true;
        return p;
    }
...
TEST_F(ConsoleDisplayTest, ShowPoint_ValidPoint_FormatsCorrectly) {
    ...
    EXPECT_NE(result.find("48.11730"), std::string::npos);
    EXPECT_NE(result.find("11.51670"), std::string::npos);
...
TEST_F(ConsoleDisplayTest, ImplementsIDisplayInterface) {
    IDisplay* idisplay = display.get();
    idisplay->showParseError("test");
    EXPECT_NE(output.str().find("test"), std::string::npos);
}
