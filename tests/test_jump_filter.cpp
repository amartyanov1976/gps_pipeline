...
protected:
    void SetUp() override {
        filter = std::make_unique<JumpFilter>(100.0); // max jump 100 meters
...
TEST_F(JumpFilterTest, Process_Enabled_SmallJump_ReturnsPass) {
    ...
    auto point = createPoint(48.1174, 11.5168); // примерно 15 метров
    auto result = filter->process(point, *history);
...
TEST_F(JumpFilterTest, Process_Enabled_LargeJump_ReturnsReject) {
    ...
    auto point = createPoint(48.1200, 11.5200); // примерно 400 метров
    auto result = filter->process(point, *history);
